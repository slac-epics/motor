...
//check for any other error
  motorError = (currentStatus_ & ( TEMP_ERROR | OVERFLOW | ARRAY_INDEX_ERROR | SYNTAX_ERROR | OVERCURRENT | EEPROM_ERROR)) ? 1:0;
  setIntegerParam(pC_->motorStatusProblem_, motorError);
//  if(motorError){
...
//  }
  ...
  skip:
//  printf("SKIP status = %d\n",(status == asynSuccess) ? 0:1);
  setIntegerParam(pC_->motorStatusCommsError_, (status == asynSuccess) ? 0:1); 
  callParamCallbacks();
  return status;
}
/*
