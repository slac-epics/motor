...
asynStatus SmartAxis5::handleError(int error){
  ...
  asynStatus status = asynSuccess;
  if(error & TEMP_MINOR){
    asynPrint(pC_->pasynUserSelf, ASYN_TRACE_ERROR, "%s:%s: Minor temperature error\n", driverName, functionName);
    pC_->setStringParam(axisNo_, pC_->smartError_, "Minor Temerature Error");
  }
  else if(error & TEMP_MAJOR){
    asynPrint(pC_->pasynUserSelf, ASYN_TRACE_ERROR, "%s:%s: Major temperature error\n", driverName, functionName);
    ...
    asynPrint(pC_->pasynUserSelf, ASYN_TRACE_ERROR, "%s:%s: Over current error\n", driverName, functionName);
