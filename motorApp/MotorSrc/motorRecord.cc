...
Version:	1.43.2.1
Modified By:	sluiter
Last Modified:	2008/11/22 20:35:04
*/
 ...
 * .44 11-22-08 rls - Prevent multiple LOAD_POS actions due to STUP's.
 *                  - Remove redundant DMOV posting from special().
 *                  - NTM logic is restored to using feedback.
...
LOGIC:
	...
	IF motor-in-motion indicator (MOVN) is true.
	    IF [New target monitoring is enabled], AND,
	       ...
	       [|Dist. to target| > 2 x (|Backlash Dist.| + Retry Deadband)], AND,
...
static long process(dbCommon *arg)
{
	...
	if (pmr->movn)
	{
	    int sign_rdif = (pmr->rdif < 0) ? 0 : 1;
            double ntm_deadband =  2.0 * (fabs(pmr->bdst) + pmr->rdbd);
